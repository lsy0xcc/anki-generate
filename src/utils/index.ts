...
export const getWordData: (word: string) => Promise<DataList> = async (
  ...
  let jpzhResult: DataList[0];
  try {
    jpzhResult = await getJpzh(word);
  } catch {
    jpzhResult = [];
  }

  let nhkResult: DataList[1];
  try {
    nhkResult = await getNhk(word);
  } catch {
    nhkResult = [];
  }
  const result: DataList = [jpzhResult, nhkResult];
  return result;
};
    ...
    result[0].forEach((jpzhItem) => {
      ...
      for (key in jpzhItem) {
        if (jpzhResult[key] && jpzhItem[key]) {
          ...
          jpzhResult[key] = `<div class="jpzh-${key.replace(
            /[A-Z]/g,
            (match) => {
